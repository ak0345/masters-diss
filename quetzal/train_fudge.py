...
def main():
    ...
    ap.add_argument("--target", choices=["y", "r"], default="y",
                     help="'y': original binary top-quartile classifier (BCE, val_acc). "
                          ...
                          "(MSE on a standardised target, val R^2/Pearson r) -- keeps the "
                          "continuous signal a binary threshold throws away.")
    args = ap.parse_args()
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            if args.target == "r":
                ...
                pred_c = chosen_logit - chosen_logit.mean()
                targ_c = y_val - y_val.mean()
                val_pearson = (pred_c * targ_c).sum() / (pred_c.norm() * targ_c.norm() + 1e-8)
                val_pearson = val_pearson.item()
