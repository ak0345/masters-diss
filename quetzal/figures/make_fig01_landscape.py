...
against N=10,000). Both are printed to stdout so the gap is auditable. This is
why the GEOM reference defaults to best-of-10,000: it matches the fine-tuned
lanes' budget. With --no_finetune (no fine-tuned lanes on the axis at all),
the reference instead defaults to best-of-5,000, matching the guides' and the
frozen prior's own sample size; override either with --ref_budget.
...
def main():
    ...
    ap.add_argument("--ref_budget", type=int, default=None,
                    help="GEOM reference sample budget. Defaults to 5000 with "
                         "--no_finetune (matching the guides' own N=5,000 "
                         "sample) and 10000 otherwise (matching the fine-tuned "
                         "lanes' 10,000-call budget).")
    fs.add_arg_common(ap, "out/fig01_landscape.pdf")
    args = ap.parse_args()
    fs.use_paper_style()
    ref_budget = args.ref_budget if args.ref_budget is not None else (
        5000 if args.no_finetune else 10000)
    ...
    for ax, bench in zip(axes, benches):
        ...
        ref = ((fs.load_reference(bench, budget=ref_budget) or harvest.get("_reference") or {})
               .get("top10"))
        if ref is not None:
            ax.axvline(ref, color=fs.REF_COLOUR, ls="--", lw=1.3, zorder=2)
            ax.text(ref, len(lanes) - 0.55, f" GEOM best-of-{ref_budget // 1000}k {ref:.3f}",
