...
def table(body, caption, label, colspec, header, note=None):
    ...
    if note:
        # \centering (set above, for the tabular) is still in effect here, so a
        # plain `{\footnotesize note}` gets centred line-by-line instead of set
        # normally -- ugly for anything longer than one line. \raggedright
        # locally overrides that within this group only.
        lines.append(r"\vspace{2pt}")
        lines.append(rf"{{\raggedright\footnotesize {note}\par}}")
...
def tab_guide_mpo(out_dir, beta="10", exclude=("zaleplon", "nitrogen")):
    ...
    for bench in sorted({k[0] for k in groups}):
        p = prior.get(bench)
        # 3 decimals to match how a top-10 score is reported everywhere else in the
        # paper (tab_landscape, the main-text prose); this caption previously used 4,
        # which read as a different, more precise number for the same quantity.
        prior_note.append(f"{fs.BENCH_TITLE.get(bench, bench)}: {fmt(p, 3)}")
...
def tab_nitrogen(out_dir, betas=("1", "10", "100")):
    ...
    for key in sorted(groups):
        ...
        fcd = np.nanmean([fs.f(r.get("fcd_guided_vs_base_mean")) for r in vals])
        if n == 0:
            # The sweep ran (a row exists per seed) but every seed came back
            # with no usable guided-sample reward, so there is nothing to
            # average -- distinct from a cell that was never run at all.
            missing.append(f"{guide}/{obj.upper()}/$\\beta={beta}$/{replay}")
            continue
    ...
    if missing:
        missing_note = (
            f" {len(missing)} cell(s) of the full grid are absent for this "
            f"reason ({', '.join(missing)}): every generated sample in both "
            r"seeds was longer than the size limit and none produced a "
            r"scoreable molecule, so no $\Delta$top-10 could be computed.")
