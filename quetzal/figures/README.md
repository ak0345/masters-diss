...
## Figures added for the chemistry analysis

| script | figure |
|---|---|
| `make_fig20_composition.py` | composed sampler by mixing operator |
| `make_fig21_composed_chemspace.py` | chemical space of the composed dumps |
| `make_fig22_composition_comparison.py` | composition against the single-objective guides |
| `make_fig23_chemistry_nulls.py` | scaffold, similarity and identical-molecule statistics, each against its null |
| `make_fig24_property_marginals.py` | physicochemical property shifts, guided against prior |

The last two read `results/ablations/chemistry/summary.json` rather than the dumps directly, so
they are instant to regenerate once that analysis has run.
