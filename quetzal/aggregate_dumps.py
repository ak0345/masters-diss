...
# compose-<benchkey>-<guide>-<objective>-<operator>-k<K>-b<beta>[-s<train_seed>]
# guide/objective identify which trained component guides were composed (e.g.
# hidden/db); train_seed (via _SEED_SUFFIX) is THEIR training seed, not the
# dump's sampling seed.
NAME_RE_COMPOSE = re.compile(
    r"^compose-(?P<bench>[^-]+)-(?P<guide>[^-]+)-(?P<objective>[^-]+)-"
    r"(?P<operator>[^-]+)-k(?P<k>\d+)-b(?P<beta>\d+)" + _SEED_SUFFIX)
# pre-existing form with no guide/objective in the name, kept so any dump
# written before this distinction existed still parses (guide/objective come
# back as "unknown"/"compose" rather than crashing the aggregator).
NAME_RE_COMPOSE_LEGACY = re.compile(
...
def parse_name(name):
    ...
    if m:
        # reward is the benchmark being composed toward; replay is always off
        # for composed guides (the composition track never trains with replay).
        return {"reward": m.group("bench"), "guide": m.group("guide"),
                "objective": m.group("objective"), "replay": "off",
                "beta": int(m.group("beta")), "train_seed": _seed_of(m),
                "family": "compose", "operator": m.group("operator"),
                "n_components": int(m.group("k"))}
    m = NAME_RE_COMPOSE_LEGACY.match(name)
    if m:
...
def main():
    ...
    front = ["name", "base_name", "train_seed", "seed", "family", "reward",
             "guide", "objective", "replay", "beta", "operator"]
    ...
    agg_cols = (["name", "base_name", "train_seed", "family", "reward", "guide",
                 "objective", "replay", "beta", "operator", "n_seeds"]
