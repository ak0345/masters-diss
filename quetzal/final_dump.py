...
def _convert_smiles_one(task):
    """(index, atoms, coords) -> (index, smiles or None). Runs in a worker.

    Same conversion as _convert_one but with no reward scoring, for callers
    that already have the reward from elsewhere (e.g. a composed sampler
    scored by gflow_multi's own reward pipeline) and only need SMILES.
    """
    idx, a, c = task
    try:
        from reward_fn import mol_to_rdkit
...
def convert_smiles_parallel(mols, procs, timeout, progress_every=0, tag=""):
    """SMILES-only counterpart to _convert_parallel, with the same wedged-worker
    protection (process pool, spawn context, between-completions timeout — see
    _convert_parallel's docstring for why SIGALRM cannot substitute for this).

    Returns (smiles_by_index, n_lost). An entry is None for a molecule that
    failed to parse or was abandoned when the pool ran out of patience.
    """
    import multiprocessing as _mp
    ...
    try:
        it = pool.imap_unordered(_convert_smiles_one, tasks, chunksize=1)
        while True:
            try:
                idx, smi = it.next(timeout=timeout)
            except StopIteration:
                break
            except _mp.TimeoutError:
                print(f"[convert{tag}] no completion for {timeout}s with "
            ...
            if progress_every and (done % progress_every == 0
                                   or done == len(tasks)):
                print(f"[convert{tag}]   {done}/{len(tasks)} converted",
                      flush=True)
            if done == len(tasks):
                break
    finally:
