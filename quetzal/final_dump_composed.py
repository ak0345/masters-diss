...
def score_block(mols, primary_logr, dataset, ref_smiles, progress_every=0, tag="",
                conv_procs=8, conv_timeout=90.0):
    ...
    SMILES conversion runs through fd2.convert_smiles_parallel, the same
    process-pool-with-timeout path final_dump.py uses: rdDetermineBonds can
    wedge inside a C++ extension that SIGALRM cannot interrupt (see
    final_dump._convert_parallel's docstring), and an in-process loop here had
    no defence against that -- one bad molecule would hang the whole dump.
    """
    smis, _n_lost = fd2.convert_smiles_parallel(
        mols, conv_procs, conv_timeout, progress_every=progress_every, tag=tag)
    smiles = [s for s in smis if s]
...
def dump_one_operator(args, operator):
    ...
    g = score_block(composed_mols, composed_scored[primary], args.dataset,
                    ...
                    conv_procs=procs, conv_timeout=args.conv_timeout)
    ...
    # guide_arch/objective identify which trained component guides were
    # composed (e.g. hidden/db); distinct configurations must not collide on
    # this name, since aggregate_dumps.py groups rows by it.
    name = (f"compose-{args.bench_key}-{args.guide_arch}-{args.objective}-"
            f"{operator}-k{K}-b{int(comp.compose_beta)}")
    if args.train_seed is not None:
        name += f"-s{args.train_seed}"
    gt = b_top = None
    ...
    summary = {
        ...
        "operator": operator,
        "product_kind": pk,
        "guide_arch": args.guide_arch,
        "guide_objective": args.objective,
...
def main():
    ...
    ap.add_argument("--guide_arch", default="na",
                    help="component guides' architecture (hidden/base), for naming")
    ap.add_argument("--objective", default="na",
                    help="component guides' training objective (db/rtb), for naming")
    ap.add_argument("--train_seed", type=int, default=None,
                    help="component guides' training seed, for naming (not the dump seed)")
    ...
    ap.add_argument("--conv_procs", type=int, default=0,
                    help="workers for SMILES conversion; 0 = cpu_count//2")
    ap.add_argument("--conv_timeout", type=float, default=90.0,
                    help="seconds of silence before abandoning a wedged worker")
