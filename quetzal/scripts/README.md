...
## Standalone analyses added after the numbered pipeline

These sit beside the numbered stages rather than inside them, because each answers one
reviewer question and none is needed to reproduce the main sweep. They live here, and not
under `results/`, because the manuscript workspace treats everything under `results/` as
read-only evidence.

```bash
# The two-component Perindopril weight line: 11 weightings x 3 mixing operators.
# Needs a GPU. NUM_GPUS runs that many points concurrently, one per visible device.
NUM_GPUS=3 bash scripts/sweep_peri_weights.sh

# Reads the sweep above. No GPU. Reports the sub-score frontier, nearest-neighbour
# similarity and novel-scaffold fraction against a matched unguided-prior control, and
# the hypervolume of the attained (c0, c1) front.
python scripts/analyse_peri_weights.py

# Emits tab_atoms_structure.tex, the atoms-versus-structure comparison across all three
# architectures. Reads the per-leg JSONs the two pilot diagnostics write.
python scripts/make_atoms_vs_structure_table.py
```

Two notes on `analyse_peri_weights.py`. It recomputes hypervolume itself rather than reading
the `hypervolume` field the composed dumps already carry: that field is computed over three
axes, one of which is a function of the other two, and it refits its score floors on each
invocation, so two weight points are normalised differently and cannot be compared. And it
deliberately reports no significance test over the weight points, because they share a
sampling seed and collapse to a handful of distinct populations, so any such test would count
one outcome many times over.
