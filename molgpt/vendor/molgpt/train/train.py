...
if __name__ == '__main__':
    ...
    # GEOM-Drugs retrain (2026-09-04): the upstream hardcoded 94-token
    # GuacaMol vocab list that used to sit here is wrong for this dataset --
    # restored the dynamic vocab computation the original authors commented
    # out, and it now also always dumps its result to a JSON manifest next
    # to the checkpoint dir, so the inference side (molgpt_prior.py) can load
    # the *exact* same stoi the trained embedding/head were built with,
    # rather than risk an independently-recomputed vocab silently
    # disagreeing on token order.
    whole_string = ' '.join(smiles + vsmiles + scaffold + vscaffold)
    whole_string = sorted(list(set(regex.findall(whole_string)) | {'<'}))
    print('Vocab size:', len(whole_string))

    import json as _json
    import os as _os
    _ckpt_dir = '../cond_gpt/weights'
    _os.makedirs(_ckpt_dir, exist_ok=True)
    with open(_os.path.join(_ckpt_dir, f'{args.run_name}_vocab_manifest.json'), 'w') as _f:
        _json.dump({'stoi': {tok: i for i, tok in enumerate(whole_string)},
                    'max_len': max_len, 'scaffold_max_len': scaffold_max_len}, _f, indent=2)
