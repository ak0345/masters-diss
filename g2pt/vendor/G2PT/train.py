...
# data preparation
# geom_drugs (added 2026-09-04): AutoTokenizer.from_pretrained(...) + tokenizer(...)
# raises "WordPiece error: Missing [UNK] token from the vocabulary" under this
# environment's tokenizers library -- see simple_tokenizer.py's docstring for
# why a plain whitespace-split + vocab lookup is exactly equivalent here (no
# real subword tokenization ever happens, every token is already atomic).
if dataset == 'geom_drugs':
    from simple_tokenizer import SimpleWordTokenizer
    tokenizer = SimpleWordTokenizer('tokenizers/geom_drugs/vocab.json', model_max_length=block_size)
else:
