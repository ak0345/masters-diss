...
def get_datasets(dataset_name, tokenizer, order='bfs'):
    ...
    if dataset_name == 'moses':
        ...
        # Added 2026-09-04 -- see datasets/prepare_geom_drugs.py's docstring
        # and shared_data/PROVENANCE.md. 15 heavy elements, exactly matching
        # Quetzal's own geom_with_h atom vocabulary minus H (author
        # instruction: both new legs must share Quetzal's own vocab).
        ATOM_TYPE = ['ATOM_B', 'ATOM_C', 'ATOM_N', 'ATOM_O', 'ATOM_F', 'ATOM_Al',
                     'ATOM_Si', 'ATOM_P', 'ATOM_S', 'ATOM_Cl', 'ATOM_As', 'ATOM_Br',
                     'ATOM_I', 'ATOM_Hg', 'ATOM_Bi']
        BOND_TYPE = ['BOND_SINGLE', 'BOND_DOUBLE', 'BOND_TRIPLE', 'BOND_AROMATIC']

        # Shapes from datasets/geom_drugs/data_meta.json (prepare_geom_drugs.py's
        # own run, 2026-09-04): 209,843 train / 23,315 eval molecules, same
        # shared_data/{train,val}.smi split MolGPT trains on.
        train_shape = {'x': (209841, 90), 'edge_index': (209841, 2, 192), 'edge_attr': (209841, 192)}
        eval_shape = {'x': (23314, 87), 'edge_index': (23314, 2, 194), 'edge_attr': (23314, 194)}
    else:
