"""train_fudge_g2pt.py -- train a FUDGE-style future-discriminator using
the exact same LogitGuide architecture and checkpoint format the DB/RTB
guides use, so it can be evaluated with the UNCHANGED final_dump_g2pt.py/
flip_g2pt.py -- no separate FUDGE generation code needed at all.
...
fudge_data_g2pt.py, only the output dimension corresponding to the token
...
`guided_logits_dispatch` -- meaning `final_dump_g2pt.py --ckpt <this
checkpoint>` and `flip_g2pt.py --ckpt <this checkpoint>` already work
...
Usage: python train_fudge_g2pt.py --data results/g2pt/fudge_data/osim.pt \
         --name fudge-osim
"""
...
def main():
    ...
    ap.add_argument("--data", required=True, help="output of fudge_data_g2pt.py")
    ap.add_argument("--name", required=True, help="checkpoint dir name, e.g. fudge-osim")
    ap.add_argument("--ckpt_root", default="logs/g2pt-gfn")
    ...
    # steps as the real DB/RTB guide sweep (scripts_g2pt/01_train_guides.sh's
    ...
    ap.add_argument("--wandb", action="store_true",
                     help="log to wandb, same project/pattern as gflow_g2pt.py's own Trainer.fit")
    ...
    if args.target == "r":
        if "r" not in blob:
            raise SystemExit(f"{args.data} has no continuous 'r' field -- regenerate with "
                              f"the current fudge_data_g2pt.py")
        r_raw = blob["r"].float()
    ...
    cfg = GFNConfig(name=args.name, model_name_or_path=args.model_name_or_path, objective="rtb",
    ...
    def save(val_loss, val_acc, epoch):
        ...
        # layer's weight+bias) so final_dump_g2pt.py needs no FUDGE-aware
    ...
    if args.wandb:
        import os
        import wandb
        wandb_run = wandb.init(project="g2pt-gfn", entity=os.getenv("WANDB_ENTITY"),
                                name=cfg.name, config=vars(args))
        ...
        with torch.no_grad():
            ...
            if args.target == "r":
                ...
                if wandb_run is not None:
                    wandb_run.log({"train/loss": train_loss, "val/loss": val_loss,
                                    "val/r2": val_r2, "val/pearson": val_pearson}, step=epoch)
            else:
                ...
                if wandb_run is not None:
                    wandb_run.log({"train/loss": train_loss, "val/loss": val_loss, "val/acc": val_acc}, step=epoch)
