...
not zero, matching flip_g2pt.py's own null-vs-zero discipline.

Reads `by_regime.overall` (not `atom_steps`) for the same reason
aggregate_g2pt.py and fig02 do: this is the raw per-position signal pooled
over the whole sequence, not a claim about the atom/non-atom split.
"""
import glob
import json
import re
...
def main():
    ...
    for f in files:
        ...
        overall = t1.get("by_regime", {}).get("overall", {})
        rate = overall.get("flip_rate_by_position")
        counts = overall.get("state_count_by_position")
        if not rate:
            continue
